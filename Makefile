.PHONY: help redis-up redis-down redis-restart redis-logs redis-cli redis-status dev dev-simple build kill-next stop-all clean clean-next install
...
dev-simple: redis-up ## Start development server without cleanup script
...
stop-all: ## Stop Redis and kill Next.js processes
