...
class RedisCacheHandler {
  ...
  async get(key) {
    ...
    try {
      ...
      const cached = await withTimeout(this.client.get(cacheKey), CONFIG.commandTimeout);
  ...
  async set(key, data, ctx) {
    try {
      ...
      const staleAge =
        ctx.revalidate && typeof ctx.revalidate === 'number' && ctx.revalidate > 0
          ? ctx.revalidate
          : CONFIG.defaultStaleAge;

      // Estimate expire age (TTL) - typically 2x stale age in production
      const expireAge =
        process.env.NODE_ENV === 'production' ? staleAge * 2 : Math.ceil(staleAge * 1.2);
  ...
  async revalidateTag(tag, options) {
    try {
      const tagKey = getTagKey(tag);
      let keys = [];
